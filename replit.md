...
- **Server-side Voice Typing with Gemini 2.5 Pro:** Audio is chunked (5-min mp3 segments) and uploaded to Gemini Files API for high-accuracy Hindi multi-speaker transcription with speaker labels (`वक्ता 1:`, `वक्ता 2:`...). Vosk small-hi remains as a legacy code path but the live engine for new jobs is Gemini. See `backend/pipeline/voice_typing/transcribe_gemini.py`.
...
- **Voice Typing requires `GEMINI_API_KEY`:** The transcription engine is now Gemini 2.5 Pro (Hindi multi-speaker). Set the key under Admin → API Keys or as an env var; the worker falls back to the DB-stored key if the env var is missing. AssemblyAI is reserved for AI Transcribe and is NOT used here.
